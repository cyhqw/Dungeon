...
export enum EffectType {
  ...
  /** 增伤 — 增加自身直接攻击伤害（加算） */
  DAMAGE_BOOST = '增伤',
  /** 无视闪避 — 下次攻击牌无视闪避，触发后层数-1 */
  IGNORE_DODGE = '无视闪避',
  /** 虚弱 — 降低自身直接攻击伤害（加算） */
  WEAKEN = '虚弱',
  /** 烙印 — 维罗妮卡的烙印，本身无直接效果 */
  BRAND_MARK = '烙印',
  /** 服从烙印 — 受击给对方施加烙印，命中后按目标烙印回复生命 */
  ...
  FATIGUE = '疲劳',
  /** 麻醉深度 — 达到阈值时触发不同效果 */
  ANESTHESIA_DEPTH = '麻醉深度',
  /** 活体房间 — 回合开始按自身点数为对手施加疲劳，并为自身施加蓄力 */
  LIVING_ROOM = '活体房间',
  /** 鳞粉 — 使用闪避卡牌时，点数增加等同层数的数值 */
  SCALE_POWDER = '鳞粉',
  /** 寒冷 — 降低造成的伤害，攻击后层数减半 */
  COLD = '寒冷',
  /** 温差 — 获得寒冷时额外增加寒冷；受到燃烧伤害时额外增伤 */
  TEMPERATURE_DIFF = '温差',
  ...
  /** 元素附加 — 每回合开始时为对手施加随机元素异常 */
  ELEMENT_ATTACH = '元素附加',
  /** 荆棘 — 每次受到直接伤害时，反弹等同于层数的伤害 */
  THORNS = '荆棘',
  /** 共损 — 将自身当前回合受到的所有非真实伤害返还给敌方，回合结束时层数-1 */
  CO_DAMAGE = '共损',
  /** 血族 — 造成直接伤害或触发对方流血伤害后，回复等量生命；受到来自对方的治疗时改为等量真实伤害 */
  BLOODLINE = '血族',
  /** 契约诅咒 — 无视一切复活手段 */
  CONTRACT_CURSE = '契约诅咒',
